...
<body id="body" class="up-scroll">
	...
	<div class="modal fade" id="login-modal" tabindex="-1" role="dialog"
		...
		<script type='text/javascript'>
								...
								Kakao.init('63e6b88bfcdd87b58fe2619ebf9cd20a'); //여기서 아까 발급받은 키 중 javascript키를 사용해준다.
								// 카카오 로그인 버튼을 생성합니다.
		...
		function checkId() {
			...
			if (!check.test(id)) {
				...
				$.post("idCheck.do", {
					...
					if (data == '0') {
						$("#checkEmailup").html("사용가능한 EMAIL 입니다.").css(
								"color", "green");
						$("#input_pwd").focus();
					} else {
						$("#checkEmailup").html("사용불가능한 EMAIL 입니다.").css(
								"color", "red");
						$("#input_email").focus();
					}
				});
	...
	<script>
		function FindIdAjax() {
			...
			if (name.length < 1 || birth.length != 10) {
				$("#checkFindMes").html("이름 또는 생년월일을 확인해주세요.").css("color",
						"red");
			} else {
				$
						.post(
		...
		<nav class="nav-menuzord nav-menuzord-transparent navbar-sticky"
			...
			<div class="container clearfix">
				<div id="menuzord" class="menuzord menuzord-responsive">
					...
					<a href="/comic/index.do" class="menuzord-brand"> <img
						alt="logo" src="/comic/resources/img/logo-blue.png"
						style="width: 180px; height: 70px; border: 2 solid white;">
						<!-- <svg
